...
本システムの `dist/output/` フォルダには、実行時に取得・リサイズされたニュース画像が保存される。GitHub Pages 上のギャラリー（`index.html`）がこれらの画像を直接参照するため、Web サイトを正常に表示させるには、これらの画像アセットをリポジトリの履歴に含めてプッシュする必要がある。
